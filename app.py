...
# Estilização CSS Premium e Românticaimport streamlit as st
import pandas as pd
import random
...
def carregar_dados():
    try:
        ...
        df_elogios = pd.DataFrame([{"Frase": "Você é o meu momento favorito do dia!"}, {"Frase": "Seu sorriso ilumina meu world."}])
...
if tela_selecionada == "🌌 Início & Carinho":
    # PROTEÇÃO AQUI: Se o nome do arquivo falhar, o app não quebra mais!
    try:
        st.image("Imagens/Pequeno Príncipe.jpg", use_column_width=True)
    except Exception:
        try:
            # Segunda tentativa caso o nome esteja sem acento ou minúsculo
            st.image("Imagens/Pequeno_Principe.jpg", use_column_width=True)
        except Exception:
            st.warning("🌌 (O banner principal está sendo calibrado no servidor!)")
...
# ==========================================
    ...
    for index, row in df_missoes.iterrows():
        ...
        if st.button(f"🔍 Abrir Detalhes da Missão: {row['Titulo']}", key=f"btn_{row['ID']}"):
            ...
            # PROTEÇÃO DO GIF: Se o nome do GIF na planilha errar a letra, avisa sem quebrar
            try:
                st.image(
st.markdown("""
    <style>
