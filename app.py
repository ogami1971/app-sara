...
st.markdown("""
    ...
    /* Estilização customizada para inputs e selectboxes do Streamlit combinarem com o tema */
...
def carregar_dados():
    ...
    # Fallbacks de segurança caso o servidor do Google Sheets falhe ou esteja offline
    if not capa: 
        capa = [{"Titulo_App": "Nosso Universo", "Subtitulo_App": "Bem-vinda, minha rosa."}]
    if not elogios: 
        elogios = [{"Frase": "Você é o meu momento favorito do dia!"}, {"Frase": "Seu sorriso ilumina meu mundo."}]
    if not missoes: 
        missoes = [
...
# --- TELA 3: NOSSO DIÁRIO ---
    ...
    st.markdown("> *“Tu te tornas eternamente responsável porilo que cativas.”*")
    try: 
