    ...
    for idx, missao in enumerate(lista_missoes):
        ...
        if ja_feita:
            ...
            if st.button(f"🎯 Concluir Desafio ({xp_ganho} XP)", key=f"btn_ctrl_{id_unico_missao}"):
                ...
                # Opcional: Descomente a linha abaixo se quiser que o bot avise no WhatsApp quando uma missão for cumprida!
                # notificacoes.disparar_notificacao_planilha(f"Completou a missão: '{titulo}' (+{xp_ganho} XP) ⭐", quem_enviou="Sara")
...
def exibir_enviar_carinho():
    ...
    with st.form("form_carinho", clear_on_submit=True):
        ...
        if botao_enviar and texto_carinho:
            ...
            # 🎯 Envia identificando o "Denner" para a mensagem ir para a Sara
            sucesso = notificacoes.disparar_notificacao_planilha(carinho_formatado, quem_enviou="Denner")
            
            if sucesso:
                st.balloons()
                st.success("✨ Carinho enviado com sucesso diretamente para o WhatsApp dela!")
            else:
                st.error("Erro ao enviar pelo CallMeBot. Verifique se as configurações e a API Key no arquivo 'notificacoes.py' estão certas.")
        ...
        with st.container():
            if cupom["usado"]:
                ...
                if botao_resgatar:
                    ...
                    # 🎯 Envia identificando a "Sara" para o WhatsApp do Denner vibrar com a notificação
